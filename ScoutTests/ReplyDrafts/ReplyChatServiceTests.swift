...
struct ReplyChatServiceTests {
    ...
    private func draft() -> ReplyDraft {
        ReplyDraft(
            ...
            contextAnswerRef: nil, bodyMarkdown: "Ahoj Priyo, ...",
            summary: "GDPR role per use-case; ball is with us.",
            relatedMessages: [DraftMessage(date: "2026-05-26", sender: "Priya", text: "tři varianty", id: "0")]
        )
    }
    ...
    @Test func promptGroundsInDraftContext() {
        ...
        #expect(p.contains("[2026-05-26] Priya: tři varianty")) // thread
        #expect(p.contains("To: Priya <p@example.com>"))
        #expect(p.contains("Cc: Sam <s@example.com>"))
        #expect(p.contains("Ahoj Priyo, ..."))                 // current reply
